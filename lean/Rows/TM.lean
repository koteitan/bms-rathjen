...
def rows : List Row := [
  ...
  { m := [[0,0],[1,1],[2,2]], t := psi (Z zero) (Z (phi zero zero)),
    ...
    note := "行 1 に 2 が現れる最初の行。旧値 φ̄(ω,0) を訂正。🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。正しい値は ψ_Ω(φ̄(1,Ω)) (§140、測定であって証明ではない)。詳細は diff.md",
  ...
  { m := [[0,0],[1,1],[2,2],[2,0],[3,1],[4,2]],
    ...
    note := "🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。正しい値は ψ_Ω(Z1 ⊕ φ̄(0, φ̄(1,Ω) ⊕ ψ_Ω(φ̄(1,Ω)))) (§140、測定であって証明ではない)。詳細は diff.md" },
  { m := [[0,0],[1,1],[2,2],[2,1]],
    t := psi (Z zero) (add (Z (phi zero zero))
  ...
  { m := [[0,0],[1,1],[2,2],[2,2]],
    ...
    note := "旧値 φ̄(ω²,0) を訂正。🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。正しい値は ψ_Ω(Z1 ⊕ φ̄(1, Ω⊕1)) (§140、測定であって証明ではない)。詳細は diff.md",
    sel := "**B** ψ₀ の引数が和になる最初" },
  { m := [[0,0],[1,1],[2,2],[2,2],[2,2]],
    t := psi (Z zero) (add (Z (phi zero zero))
      (add (Z (phi zero zero)) (Z (phi zero zero)))),
    name := "\\psi_0(\\Omega_2\\cdot 3)",
    note := "🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。正しい値は ψ_Ω(Z1 ⊕ Z1 ⊕ φ̄(1, Ω⊕1⊕1)) (§140、測定であって証明ではない)。詳細は diff.md" },
  { m := [[0,0],[1,1],[2,2],[3,0]],
  ...
  { m := [[0,0],[1,1],[2,2],[3,0],[4,1],[5,2]],
    ...
    note := "🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。正しい値は ψ_Ω(φ̄(0, Z1 ⊕ ψ_Ω(φ̄(1,Ω)))) (§140、測定であって証明ではない)。詳細は diff.md" },
  { m := [[0,0],[1,1],[2,2],[3,1]],
