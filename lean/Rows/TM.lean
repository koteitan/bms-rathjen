...
**状態: Lean に存在しない。** 定義も定理の記述も無く、上の式はこの文書の中の文である。
順序型に mathlib が要り、$`\\mathrm{wf}`$ は仮定として切り出す (紙の上では Rathjen 1994
の整列証明として既知)。以下の E は、これを支えるため、あるいはこれを迂回して同じことを
言うために立てられている。
...
**状態: Lean の定理。11 行で証明済み。** 較正事故のあと、$`o`$ に言及しない形へ移した結果である
...
**状態: Lean の定理。部分的。** $`\\mathfrak{T}(M)`$ 側の共終性は証明書の中で行ごとに
使われている。
...
**状態: 一般形は Lean に存在しない。** 順序は決定可能なので、個別のペアは計算で即検査できる。
