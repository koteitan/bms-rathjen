...
- **証明列**: ✅ = [意味証明書](../lean/Evidence/Cert.lean) `Certified M t`
  ...
  ✅ が主張することとしないこと (v0.1.78 時点、すべて定理):

  - **上限**: `certRows_no_overshoot` — この行の証明書が ω^(t+1) 以上の
    値を取ることはない。ε₀ の行はさらに鋭く、ε₀ より上の値は一切
    認証され得ない (`no_cert_above_eps0`)。いずれも証明書の内部に現れる
    部分値に何の制限も課さない形で証明されている。
  - **一意性 (守り付き)**: `certRows_unique_guarded` — 値が遺伝的に
    $`T(M)`$ の項である証明書 (`CertifiedIn DomF`) の範囲では、登録値が唯一。
  - **まだ主張しない**: 登録値より**下**の値の棄却。これは証明書の値を経由する
    推移律を要するため、ψ/Z 領域の順序理論 (Stage 3b) 待ちである。
  - なお守りが必要なことは定理で示されている: `cert_not_single_valued` により
    `(0)(1)` は ω と 1+M の両方の証明書を持つ (1+M は $`T(M)`$ の項ではない)。
    証明書は守りなしでは一価でない。
