...
def rows : List Row := [
  ...
  { m := [[0,0],[1,1],[2,2]], t := psi (Z zero) (Z (phi zero zero)),
    ...
    note := "(h?) 🚨 **値は疑わしい** — 展開の値の上限がこの値に届かない。`ψ_Ω(φ̄(1,Ω))` が正しいという証拠が 3 つある。[findings.md](findings.md)",
  ...
  { m := [[0,0],[1,1],[2,2],[2,0],[3,1],[4,2]],
    ...
    note := "(h?) 🚨 **値は疑わしい** — 展開の値の上限がこの値に届かない。差し替え先は未決。[findings.md](findings.md)" },
  { m := [[0,0],[1,1],[2,2],[2,1]],
    t := psi (Z zero) (add (Z (phi zero zero))
  ...
  { m := [[0,0],[1,1],[2,2],[3,0],[4,1],[5,2]],
    ...
    note := "🚨 **値は疑わしい** — 展開の値の上限がこの値に届かない。`ψ_Ω(φ̄(0, Z1 ⊕ ψ_Ω(φ̄(1,Ω))))` が候補。[findings.md](findings.md)" },
  { m := [[0,0],[1,1],[2,2],[3,1]],
