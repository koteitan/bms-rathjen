...
def rows : List Row := [
  ...
  { m := [[0,0],[1,1]], t := e0, name := "\\varepsilon_0", proof := "R1",
    hasO := true, ev := "bisim6", note := "(h) 2 行の最初の極限",
  ...
  { m := [[0,0],[1,1],[2,0],[1,1],[1,0],[2,1],[3,0],[1,0],[2,1]],
    ...
    proof := "namespace F1", note := "(h≠) 外部の表と食い違うが決着済み。当方が正しい ([diff.md](diff.md) 族 1)",
  ...
  { m := [[0,0],[1,1],[2,1],[1,1],[2,0],[3,1],[4,1],[3,1],[1,1],[2,0]],
    ...
    proof := "namespace F2a", note := "(h≠) 外部の表と食い違うが決着済み。当方が正しい ([diff.md](diff.md) 族 2)",
    sel := "**D** 外部の表と食い違う 9 行の 1 つ" },
  { m := [[0,0],[1,1],[2,1],[1,1],[2,0],[3,1],[4,1],[3,1],[1,1],[2,0],[3,1]],
    ...
    proof := "namespace F2b", note := "(h≠) 外部の表と食い違うが決着済み。当方が正しい ([diff.md](diff.md) 族 2)",
    sel := "**D** 外部の表と食い違う 9 行の 1 つ" },
  ...
  { m := [[0,0],[1,1],[2,1],[1,1],[2,0],[3,1],[4,1],[3,1],[4,0],[5,1],[6,1],[5,0]],
    ...
    proof := "namespace F3a", note := "(h≠) 外部の表と食い違うが決着済み。当方が正しい ([diff.md](diff.md) 族 3)",
  ...
  { m := [[0,0],[1,1],[2,1],[1,1],[2,0],[3,1],[4,1],[3,1],[4,0],[5,1],[6,1],[5,0],[6,1]],
    ...
    proof := "namespace F3b", note := "(h≠) 外部の表と食い違うが決着済み。当方が正しい ([diff.md](diff.md) 族 3)",
    sel := "**D** 外部の表と食い違う 9 行の 1 つ。当方が大きい側" },
  { m := [[0,0],[1,1],[2,1],[1,1],[2,0],[3,1],[4,1],[3,1],[4,0],[5,1],[6,1],[5,0],[6,1],[7,1]],
    ...
    proof := "namespace F3c", note := "(h≠) 外部の表と食い違うが決着済み。当方が正しい ([diff.md](diff.md) 族 3)",
    sel := "**D** 外部の表と食い違う 9 行の 1 つ。当方が大きい側" },
  ...
  { m := [[0,0],[1,1],[2,1],[2,0]],
    ...
    name := "\\zeta_\\omega", note := "(h) 旧値 ε_{ζ₀·ω} を訂正 (較正事故)" },
  { m := [[0,0],[1,1],[2,1],[2,1]],
    t := phi (add (phi zero zero) (add (phi zero zero) (phi zero zero))) zero,
    name := "\\bar{\\varphi}(3,0)", note := "(h) 旧値 ζ₁ を訂正 (較正事故の初検出行)" },
  ...
  { m := [[0,0],[1,1],[2,1],[2,1],[2,0],[1,1]],
    ...
    proof := "namespace G9", note := "(h≠) 外部の表と食い違う ([diff.md](diff.md) 族 4)",
    sel := "**D** 外部の表と食い違う 9 行の 1 つ" },
  { m := [[0,0],[1,1],[2,1],[2,1],[2,0],[1,1],[2,1]],
    ...
    proof := "namespace G10", note := "(h≠) 外部の表と食い違う ([diff.md](diff.md) 族 4)",
    sel := "**D** 外部の表と食い違う 9 行の 1 つ" },
  { m := [[0,0],[1,1],[2,1],[2,1],[2,0],[1,1],[2,1],[2,1]],
    ...
    note := "(h≠) 外部の表と食い違う ([diff.md](diff.md) 族 4)",
    sel := "**D** 外部の表と食い違う 9 行の 1 つ" },
  ...
  { m := [[0,0],[1,1],[2,2]], t := psi (Z zero) (Z (phi zero zero)),
    ...
    note := "(h?) 行 1 に 2 が現れる最初の行。旧値 φ̄(ω,0) を訂正。🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。差し替えの候補は ψ_Ω(φ̄(1,Ω))。**Hexirp 氏・naruyoko 氏・§69 の `sbad` の三者一致** (§140、測定であって証明ではない)。詳細は diff.md",
  ...
  { m := [[0,0],[1,1],[2,2],[2,0],[3,1],[4,2]],
    ...
    note := "(h?) 🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。差し替えは**未決**。naruyoko 氏の基本列は ψ_Ω(Z1 ⊕ φ̄(0, φ̄(1,Ω) ⊕ ψ_Ω(φ̄(1,Ω)))) を指し (§140)、Hexirp 氏の表は Z1 の加数を持たない ψ_Ω(φ̄(0, φ̄(1,Ω) ⊕ ψ_Ω(φ̄(1,Ω)))) を与える。外部 2 つが食い違う。詳細は diff.md" },
  { m := [[0,0],[1,1],[2,2],[2,1]],
    t := psi (Z zero) (add (Z (phi zero zero))
  ...
  { m := [[0,0],[1,1],[2,2],[2,2]],
    ...
    note := "(h?) 旧値 φ̄(ω²,0) を訂正。🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。差し替えは**未決**。naruyoko 氏の基本列は ψ_Ω(Z1 ⊕ φ̄(1, Ω⊕1)) を指し (§140)、Hexirp 氏の表は ψ_Ω(φ̄(1, Ω⊕1)) を与える。外部 2 つが食い違う。詳細は diff.md",
    sel := "**B** ψ₀ の引数が和になる最初" },
  { m := [[0,0],[1,1],[2,2],[2,2],[2,2]],
    t := psi (Z zero) (add (Z (phi zero zero))
      (add (Z (phi zero zero)) (Z (phi zero zero)))),
    name := "\\psi_0(\\Omega_2\\cdot 3)",
    note := "(h?) 🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。差し替えは**未決**。naruyoko 氏の基本列は ψ_Ω(Z1 ⊕ Z1 ⊕ φ̄(1, Ω⊕1⊕1)) を指し (§140)、Hexirp 氏の表は ψ_Ω(φ̄(1, Ω⊕1⊕1)) を与える。外部 2 つが食い違う。詳細は diff.md" },
  { m := [[0,0],[1,1],[2,2],[3,0]],
    t := psi (Z zero) (phi zero (Z (phi zero zero))),
    name := "\\psi_0(\\psi_2(1))", proof := "namespace G2",
    note := "(h?) 旧値 φ̄(ω^ω,0) を訂正",
  ...
  { m := [[0,0],[1,1],[2,2],[3,0],[4,1],[5,2]],
    ...
    note := "🚨 **値は疑わしい**。§139 が仮定ゼロで反証 — 掲載値はこの行列自身の展開の値の上限より真に上にある。差し替えの候補は ψ_Ω(φ̄(0, Z1 ⊕ ψ_Ω(φ̄(1,Ω)))) (§140、測定)。**この行は Hexirp 氏の表に無い**ので照合は naruyoko 氏の 1 つだけ。詳細は diff.md" },
  { m := [[0,0],[1,1],[2,2],[3,1]],
