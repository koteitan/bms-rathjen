...
- [**Γ₀ より上の 23 行について — 値を信用しないこと**](#γ₀-より上の-23-行について--値を信用しないこと)
...
一致するはずだと考えて $`\\mathfrak{T}(M)`$ 側から列を選ぶ、というのがこの誤りの形で、
較正事故もこの形だった。
...
主張**だが、**それは定理ではなく測定**である — `Trans/Dict.lean` の定理は 3 本 (構造展開のみ)
に対し `#guard` が 49 個で、順序保存も単射性もそちら側にある。ファイル自身が
「candidate / 予想 tier」と名乗っている。
...
その外では `oR` は `none` を返す。表の 51 行のうち 50 行がこの範囲に収まり
(高さ 1 が 7 行、高さ 2 が 43 行)、残る 1 行は空行列で `oR [] = some 0` の枝が扱う。
したがって値は影響を受けないが、
**表がこの断片の外へ伸びることは、いまの `oR` ではできない。**

## Γ₀ より上の 23 行について — 値を信用しないこと

**この節の内容は 2026-08-12 に判明したもので、当該行の値はまだ直っていない。**

`(0,0)(1,1)(2,2)` 以上の行の値は `oR = (1+\\cdot) \\circ \\mathrm{dict} \\circ \\mathrm{oRB}`
から来ている。この `dict` について、次の 2 つが証明された (`lean/Trans/Dict.lean` §4)。

**1. 外部資料と食い違う。** BMS `(0,0)(1,1)(2,2)` の Buchholz 値 $`\\psi_0(\\psi_2(0))`$ には
...
**2. 順序を保存しない。これは資料に依存しない。** `dict` はファイル自身が「像への順序同型」を
目的として掲げているが、標準形の Buchholz 項 3174 個 (サイズ ≤ 7、添字 ≤ 3、和 ≤ 3 項) の
順序対 5035551 組を走査したところ、**281 組で順序が反転**した。像が $`\\mathfrak{T}(M)`$ の
外へ出たものは 0 件なので、故障は反転であって逸脱ではない。したがって `dict` は
**外部資料の正否とは無関係に、翻訳として失格**である。
...
当該 23 行はその $`\\Omega_2`$ が要る領域にある。値を直すには型を変える必要があり、
それは $`\\mathfrak{T}(M)`$ の項型とその上のすべてに波及するので、まだ着手していない。

**したがって、これらの行の値は誤っている可能性が高い。** ✅ の付いた行は影響を受けない —
