...
/-
Evidence/RegionNext3.lean — THE GATES ROW 326 STILL WAITS ON (§80-§94)

`RegionNext2` took the region as far as `Hclosed`, `Hzero`, the value, and the order
theory (§61-§79).  This file is the first half of the gate work and is settled: the
K-gate through §92, the Veblen fold's split (§93), and the density gate's shape (§94).
`RegionNext4` carries §95 onwards.
-/
