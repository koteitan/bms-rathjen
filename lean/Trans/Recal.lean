...
wholesale against the reference implementation CLI, and the checks below do exactly that: the Buchholz side
...
fragment but its output is only claimed to mean anything where the reference implementation applies.
...
Deviations from the JS, all deliberate:
  ...
  * every loop is `fuel`-bounded (the JS loops are `while`s whose termination is part of
    ...
    the value, and §5 pins every value against the reference implementation;
...
/-- The recalibrated reading, Buchholz side (the reference implementation's own target). -/
...
/-- The recalibrated reading: oR = (1 + ·) ∘ dict ∘ Trans.
    ...
    verbatim on the whole region where `o?` was confirmed against the reference (checked below). -/
...
/-! ## 5. Acceptance record
  ...
  (B) the reference-implementation table: `oRB` against verbatim `pss2bp --raw` output, wholesale;
  ...
  (D) every produced term satisfies `inT` ([Rathjen, 1991] 2.1).
...
and prefixes: zero mismatches against the reference implementation. -/
...
Each is a value the miscalibrated `o?` produced and the reference implementation refutes.  They are
...
-- the ζ₁ mistake: (0,0)(1,1)(2,1)(2,1) is NOT ζ₁ (o? said φ̄(2,1); reference implementation: D_0 D_1(Ω_1×2))
#guard oR [[0,0],[1,1],[2,1],[2,1]] != some (phi (ofNat 2) one)
-- the ε_{ζ₀·ω} mistake: (0,0)(1,1)(2,1)(2,0) (o? said φ̄(1,ω̄^{ζ₀+1}); reference implementation: D_0 D_1(Ω_1+1))
#guard oR [[0,0],[1,1],[2,1],[2,0]] != some (phi one (phi zero (phi (ofNat 2) zero)))
-- the φ̄(ω,0) mistake: (0,0)(1,1)(2,2) (o? said φ̄(ω,0); reference implementation: D_0 Ω_2)
...
/-! ### (B) the reference-implementation table

Every row of `table/refimpl-audit-2026-08-09.txt`.  The right-hand side is verbatim
...
`bnd` is the withdrawal boundary of v0.1.41: `o?` was confirmed against the reference implementation
