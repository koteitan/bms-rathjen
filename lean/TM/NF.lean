...
[Rathjen, 1991] 2.2 (K_κ), 2.1 (formation), 2.6 (the normal operations +, φ, ω^·).
-/
import TM.Order
...
/-- The coefficient set K_κ γ ([Rathjen, 1991] 2.2).  Finite sets are lists (duplicates harmless). -/
...
/-- The formation conditions of 𝔗(M) ([Rathjen, 1991] 2.1 and Remark (ii)):
...
/-! ## Normal operations ([Rathjen, 1991] 2.6) -/
...
/-- α + β ([Rathjen, 1991] 2.6(ii)): drop the trailing components of α that are smaller than the
...
/-- 1 := φ̄00, ω := φ̄01, Ω₁ := Z0 ([Rathjen, 1991] 2.6(v)). -/
def one : Term := phi zero zero
def omega : Term := phi zero one
def Om : Term := Z zero

/-- The term for a natural number `n` ([Rathjen, 1991] 2.6(v)). -/
def ofNat : Nat → Term
  | 0 => zero
...
/-- Default case of φαβ (last two lines of [Rathjen, 1991] 2.6(vi)): α when β = 0 and α ∈ SC,
...
/-- The case "β = γ + n + 1 with γ a fixed-point shape" of φαβ
    (middle two lines of [Rathjen, 1991] 2.6(vi)): step down to φ̄α(γ + n). -/
...
/-- φαβ ([Rathjen, 1991] 2.6(vi)): the normalized Veblen function, which re-counts fixed points.
    Assumes α, β < M. -/
