...
>
> **より広い注意。** `TM/Terms.lean` の冒頭 (2026-08-12 記) が既に
> 「`Z(1)` は χ₁(0) = I であって $`\Omega_2`$ ではない。$`(0,0)(1,1)(2,2)`$ 以上の行は
> $`\Omega_2`$ を要する領域に居るので、この型では値を正しく書けない」と述べている。
> したがって通称欄の $`\psi_0(\Omega_2)`$ という読みは、破れが測定された 5 行に限らず
> `Z(1)` を含む行すべてについて疑わしい。χ を 2 引数に戻す修理は `Term` を変える。
>
