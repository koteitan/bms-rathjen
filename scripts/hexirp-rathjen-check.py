...
すぎない。ディレクトリを渡して読む。無ければ理由を表示して飛ばす。
  ...
  Hexirp「バシク行列システム４と全部盛りオクフ・ベータの対応」
      ...
      **行き先は Rathjen の 𝔗(M) ではなく「全部盛り」の OCF である**が、
      **Rathjen 由来の部分では Rathjen と一致するはず**、と著者が述べている。したがって
      重なる範囲での食い違いは系の違いではなく本物の食い違いであり、突き合わせる意味がある。
      `W(x)` が訳せないのはその重なりの外で、先方には Ω 階層があり 𝔗(M) には無いという
      系の違いによる。当方が数えているのは両者が重なる Veblen 断片に限られる。
...
CANON = "https://googology.fandom.com/ja/wiki/ユーザー:Hexirp/ブログ"
