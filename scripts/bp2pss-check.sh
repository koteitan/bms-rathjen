...
if [ "$nguard" -gt 0 ]; then
  ...
  echo
  echo "  Every one of these is the STANDARDNESS clause, not t < Omega.  The cause is"
  echo "  almost certainly a STALE common.js under \$PSS2BP.  Before the upstream"
  echo "  commit 'Fix G(a,u) for isStandardBuchholz', G on a formal sum was written"
  echo "  a.flatMap(G), and Array.prototype.flatMap passes (element, INDEX, array),"
  echo "  so G's level argument was replaced by the component's index.  The probe:"
  ...
  echo "  A fixed G answers [] at level 5.  With the old one isStandardBuchholz"
  echo "  over-rejects psi_1(eps_0*2) and friends -- and, when the offending"
  echo "  component is not the FIRST one, under-rejects instead: it calls"
  echo "  psi_0(Omega+zeta_0) standard, and TransRev then fails downstream looking"
  echo "  for a fundamental sequence.  Both directions are the same bug.  Update"
  echo "  \$PSS2BP/common.js and re-run before treating this as a real mismatch."
