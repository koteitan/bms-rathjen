...
//     φ̄(a,b)           ↔ phi^0_{a}(β°)           β° は [R91] 2.7 の意味上の引数。
//                                                 **ここで訳に使う β° は repo の
//                                                 `phiShifted` ではない。** repo のそれは
//                                                 2.7 の逐語転記で第 2 選言が
//                                                 `b == zero && a.isSC` であり、そのまま
//                                                 使うと φ̄(Ω,0) と φ̄(Ω,1) が同じ φ_Ω(1)
//                                                 に潰れる (どちらも `inT`、当方の順序は
//                                                 両者を区別する)。訳には
//                                                 `isFP a (splitFin b).1 || a.isSC` を使う。
//                                                 どちらが [R91] の意図かは未決で、
//                                                 `TM/FS.lean` の `phiShifted` に経緯を、
//                                                 `Trans/DictInv.lean` に到達範囲の測定を
//                                                 置いた。R1 の測る範囲には届かない。
...
//
// **ただしこれは `phiShifted` が誤りだという証拠ではない** (2026-08-13 に訂正)。
// 先方は P進大好きbot 氏の拡張であって [R91] そのものではないので、この一致が示すのは
// 「訳にはこの β° を使え」までである。[R91] 2.7 の本文は逐語では φ̄α0 と φ̄α1 を同じ
// 順序数に送っており、それは 2.8 の F と両立しない — その未決の食い違いは
// `TM/FS.lean` の `phiShifted` に書いた。
