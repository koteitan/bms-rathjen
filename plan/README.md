...
$$\forall n.\ \ o(M_i[n]) = t_i[n]$$

を証明する。一般形 (長期目標) は、任意の標準形の極限行列 $M$ について

$$\forall n.\ \ o(M[n]) = o(M)[n].$$
...
「$<_T$ の $\mathrm{NF}$ 上への制限が整礎」という仮定を $\mathrm{wf}$ と書く。
...
になり、E3 で $n$ 番目同士が対応し、帰納法の仮定で順序型が一致する(零・後続も同様)。
...
- BMS 側の共終性 ($N <_B M \implies \exists n.\ N <_B M[n]$) が G の最難部で、
