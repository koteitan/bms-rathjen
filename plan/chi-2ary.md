...
> **追記 (2026-08-20)。採った (b) は通らないことが測定と証明で出た。**
> `Evidence/RegionNext8.lean` §143 が `dictB143` として (b) を試作した。表の上では
> 動く (23 行が動き、全部 `inT`、昇順、相異なる) が、**1 段下で必ず壊れる**:
>
> ```
> reg 2 = Z 1 のまま     psi_1(Om_2) と Om_2 が同じ項になる。順序の反転 672/284/1993 件
>                        (現行の dict は 0/0/0)。単射も破れる
> reg 2 = phi(1,Om)      isR (phi(1,Om)) = false で [R91] 2.1(vi) を満たさない。
>                        11577 項中 4164 項が 𝔗(M) の外に出る
> ```
>
> **そして障害は本文が原因として挙げたその事実である** — 𝔗(M) が `Om_2` を
> **正則として名指せない**。本文は「正しい値は型の中に存在する」と書いたが、
> 存在するのは**値**であって**役割 (潰す先の正則)** ではない。`collapse 1` の尾の枝が
> `Om` より上の ε 数の上で恒等写像であるために、代役 `phi(1,Om)` は
> `psi_1(Om_2)` と `Om_2` を潰す。代役が他に選べないことも測ってある
> (候補 18549 個中、段 1 以下の像を全部超えるのは `phi(1,Om)` ただ 1 個)。
> `dict_sum` も失われる (証明済み。`Evidence/Cert.lean:11538` が使っている)。
>
> **したがって (a) と (b) の二択は成り立っていない。** (b) を通すには
> `collapse` の段 1 以上を書き換えることになり、そこまで行くなら (a) の
> 移行手順 (下記) と分量が変わらない。R1 で改修しないという結論自体は変えないが、
> **「(b) なら安く直る」という前提は取り消す。**
>
> 一方で `TM/FS.lean` の側の修理は通った (§144)。そちらは `fsN` の呼び出し側 1 箇所で、
> 変わるのは §137 の 5 行ちょうど、E3 はどの行も乗り降りしない。
